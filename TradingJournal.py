...
# MIT LÖSCHFUNKTION FÜR EINZELNE TRADES
# ============================================================
...
def main():
    ...
    # ============================================================
    # 5.7 ALLE TRADES (Tabelle mit Lösch-Buttons)
    # ============================================================
    
    st.markdown('<p class="section-label">📋 Alle Trades</p>', unsafe_allow_html=True)
    
    # Tabelle mit Lösch-Buttons rendern
    for idx, row in df.iterrows():
        col1, col2, col3, col4, col5, col6, col7 = st.columns([0.5, 1.2, 1.2, 1, 1.2, 1.2, 0.8])
        ...
        with col5:
            st.write(f"{row['risk_reward_ratio']:.1f}")
        with col6:
            pnl_color = "green" if row['pnl_percent'] >= 0 else "red"
            st.markdown(f"<span style='color: {pnl_color}; font-weight: 600;'>{row['pnl_percent']:+.1f}%</span>", unsafe_allow_html=True)
        with col7:
