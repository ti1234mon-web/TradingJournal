...
def get_all_trades():
    """Lädt alle Trades aus der Datenbank."""
    conn = sqlite3.connect(DB_PATH)
...
def clear_db():
    """Löscht alle Trades (nur für Debugging)."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def calculate_metrics(df):
    """Berechnet die wichtigsten KPIs."""
    if df.empty:
        return {
            "total_pnl": 0.0,
            "total_trades": 0,
...
def main():
    ...
    # ============================================================
    # 5.1 EINGABE (Neuer Trade)
    # ============================================================
    
    with st.container():
        st.markdown('<div class="input-area">', unsafe_allow_html=True)
        st.markdown('<p style="color: rgba(255,255,255,0.4); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 1rem;">➕ Neuen Trade erfassen</p>', unsafe_allow_html=True)
    ...
    # ============================================================
    # 5.4 BALKENDIAGRAMM (PnL pro Trade)
    # ============================================================
    
    st.markdown('<p style="color: rgba(255,255,255,0.4); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 0.5rem;">📊 PnL (%) pro Trade</p>', unsafe_allow_html=True)
    
    fig = go.Figure()
    
    # Jeder Trade als Balken (grün für Gewinn, rot für Verlust)
    ...
    st.markdown('<p style="color: rgba(255,255,255,0.4); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 0.5rem;">📈 Kumulativer PnL (%)</p>', unsafe_allow_html=True)
    ...
    # ============================================================
    # 5.6 ALLE TRADES (Tabelle)
    # ============================================================
    
    st.markdown('<p style="color: rgba(255,255,255,0.4); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 0.5rem;">📋 Alle Trades</p>', unsafe_allow_html=True)
    ...
    # Debug: Datenbank leeren (versteckt)
    # with col2:
    #     if st.button("🗑️ Alle Trades löschen (Debug)"):
    #         clear_db()
    #         st.rerun()
